(function () {
  ...
  var primary = config.primaryColor || '#7c3aed';
  var secondary = config.secondaryColor || '#4338ca';
  var chatBg = config.bgColor || '#0d0d0d';

  // Derive a lighter version of primary for bot message background
  ...
  var primaryRgb = hexToRgb(primary.length === 7 ? primary : '#7c3aed');

  var css = '#cb-bubble{position:fixed;bottom:24px;right:24px;width:56px;height:56px;background:linear-gradient(135deg,' + primary + ',' + secondary + ');border-radius:50%;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 24px rgba(' + primaryRgb + ',.45);z-index:9999;transition:transform .2s}#cb-bubble:hover{transform:scale(1.1)}#cb-bubble svg{width:26px;height:26px;fill:#fff}#cb-win{position:fixed;bottom:96px;right:24px;width:360px;height:520px;background:' + chatBg + ';border:1px solid rgba(255,255,255,.1);border-radius:20px;display:none;flex-direction:column;z-index:9998;overflow:hidden;box-shadow:0 24px 60px rgba(0,0,0,.6)}#cb-win.open{display:flex}#cb-head{padding:16px 20px;background:linear-gradient(135deg,' + primary + ',' + secondary + ');color:#fff;font:600 14px/1 sans-serif;display:flex;align-items:center;gap:10px}#cb-head-dot{width:8px;height:8px;border-radius:50%;background:#4ade80;animation:cb-pulse 2s infinite}@keyframes cb-pulse{0%,100%{opacity:1}50%{opacity:.4}}#cb-msgs{flex:1;overflow-y:auto;padding:16px;display:flex;flex-direction:column;gap:10px;font-family:sans-serif}#cb-msgs::-webkit-scrollbar{width:4px}#cb-msgs::-webkit-scrollbar-thumb{background:rgba(255,255,255,.1);border-radius:4px}.cb-msg{max-width:82%;padding:10px 14px;border-radius:14px;font-size:13px;line-height:1.55}.cb-bot{background:rgba(255,255,255,.07);color:rgba(255,255,255,.88);align-self:flex-start;border-bottom-left-radius:4px}.cb-user{background:linear-gradient(135deg,' + primary + ',' + secondary + ');color:#fff;align-self:flex-end;border-bottom-right-radius:4px}.cb-typing{display:flex;gap:4px;align-items:center;padding:12px 14px}.cb-typing span{width:6px;height:6px;border-radius:50%;background:rgba(255,255,255,.4);animation:cb-bounce .9s infinite}.cb-typing span:nth-child(2){animation-delay:.15s}.cb-typing span:nth-child(3){animation-delay:.3s}@keyframes cb-bounce{0%,60%,100%{transform:translateY(0)}30%{transform:translateY(-6px)}}#cb-form{display:flex;padding:12px;gap:8px;border-top:1px solid rgba(255,255,255,.08)}#cb-inp{flex:1;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.1);color:#fff;padding:10px 14px;border-radius:10px;font-size:13px;outline:none;transition:border-color .2s}#cb-inp:focus{border-color:rgba(' + primaryRgb + ',.5)}#cb-inp::placeholder{color:rgba(255,255,255,.28)}#cb-btn{background:linear-gradient(135deg,' + primary + ',' + secondary + ');color:#fff;border:none;border-radius:10px;padding:10px 16px;cursor:pointer;font-size:18px;line-height:1;transition:opacity .2s}#cb-btn:hover{opacity:.85}';
