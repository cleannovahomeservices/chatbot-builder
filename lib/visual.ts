...
export async function analyzeWebsite(url: string): Promise<VisualAnalysis> {
  ...
  try {
    ...
    const [ssRes] = await Promise.all([
      fetch(ssUrl, { signal: AbortSignal.timeout(18_000) }),
    ]);

    if (!ssRes.ok) {
      console.error('[visual] screenshot failed:', ssRes.status);
      return FALLBACK;
    }

    const imageBuffer = await ssRes.arrayBuffer();
